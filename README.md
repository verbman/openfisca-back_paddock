...
These elements are described in different folders. All the modelling happens within the `openfisca_back_paddock` folder.
...
```sh
pip install openfisca-back_paddock
```
...
```sh
git clone git://github.com/verbman/openfisca-back_paddock.git.git
cd openfisca-back_paddock.git
...
```sh
openfisca serve --port 5000 --country-package openfisca_back_paddock
```
...
curl -X POST -H "Content-Type: application/json" \
  -d @./openfisca_back_paddock/situation_examples/couple.json \
